...
export const isValidEmail = (value = '', options = {}) => {
  const { enforceRealAddress = true } = options
  const email = normalizeEmail(value)
  ...
  if (enforceRealAddress && DISALLOWED_EMAIL_TOKENS.some((token) => domainName.includes(token) || localPart.includes(token))) return false
  return true
}
...
export const buildUserRegistrationDocument = (payload, id) => ({
  ...
  password: payload.authMethod === 'google' ? '' : (payload.passwordHash || payload.password),
  name: payload.name,
  phone: payload.phone || '',
  role: payload.role,
  ...
  emailVerified: payload.emailVerified ?? payload.authMethod === 'google',
